...
{
    public class DesignsSupplyRequestServiceTests : TestBase
    {
        [Test]
        public async Task GetAllDesignsSupplyRequestsTest()
        {
            // Arrange
            var context = await GetInitDbContext();
            
            // Act
            var designService = new DesignsSupplyRequestService(new EFUnitOfWork(context));
            var result = await designService.GetDesignSupplyRequestsAsync();
        ...
        [Test]
        public async Task GetDesignSupplyRequestsTest()
        {
            // Arrange
            var context = await GetInitDbContext();
            ...
            // Act
            var designService = new DesignsSupplyRequestService(new EFUnitOfWork(context));
            var result = await designService.GetDesignSupplyRequestAsync(1);
            
            // Assert
            Assert.NotNull(result);
            Assert.AreEqual(1, result.DesignId);
        }
        
        [Test]
        public async Task DeleteDesignSupplyRequestTest()
        {
            // Arrange
            var context = await GetInitDbContext();
            ...
            // Act
            var designService = new DesignsSupplyRequestService(new EFUnitOfWork(context));
            await designService.DeleteDesignSupplyRequestAsync(1);
            
            // Assert
            Assert.AreEqual(2, context.DesignsSupplyRequests.Count());
        }
        
        [Test]
        public async Task UpdateDesignSupplyRequestTest()
        {
            // Arrange
            var context = await GetInitDbContext();
            ...
            // Act
            var designService = new DesignsSupplyRequestService(new EFUnitOfWork(context));
            await designService.UpdateDesignSupplyRequestAsync(new DesignsSupplyRequestDTO {Id = 3, TaskId = 1, DesignId = 3, Comment = "5", Quantity = 30, StatusSupply = StatusSupplyEnumDTO.Ready});
            
            // Assert
            var updatedItem = context.DesignsSupplyRequests.FirstOrDefault(c => c.Id == 3) ?? throw new NullReferenceException("updateItem");
            Assert.AreEqual("5", updatedItem.Comment);
            Assert.AreEqual(StatusSupplyEnum.Ready, updatedItem.StatusSupply);
        }
        
        [Test]
        public async Task CreateDesignSupplyRequestTest()
        {
            // Arrange
            var context = await GetInitDbContext();
            ...
            // Act
            var designService = new DesignsSupplyRequestService(new EFUnitOfWork(context));
            await designService.CreateDesignSupplyRequestAsync(new DesignsSupplyRequestDTO {TaskId = 1, DesignId = 4, Comment = "5", Quantity = 30, StatusSupply = StatusSupplyEnumDTO.Ready});
            
            // Assert
            var createdItem = context.DesignsSupplyRequests.Last() ?? throw new NullReferenceException("updateItem");
            Assert.AreEqual(4, context.DesignsSupplyRequests.Count());
        ...
        private async Task<ApplicationContext> GetInitDbContext()
        {
            ...
            context.DesignsSupplyRequests.Add(new DesignsSupplyRequest {Id = 1, TaskId = 1, DesignId = 1, Comment = "1", Quantity = 10, StatusSupply = StatusSupplyEnum.Ordered});
            context.DesignsSupplyRequests.Add(new DesignsSupplyRequest {Id = 2, TaskId = 1, DesignId = 2, Comment = "2", Quantity = 20, StatusSupply = StatusSupplyEnum.NotAccepted});
            context.DesignsSupplyRequests.Add(new DesignsSupplyRequest {Id = 3, TaskId = 1, DesignId = 3, Comment = "3", Quantity = 30, StatusSupply = StatusSupplyEnum.NotAccepted});
